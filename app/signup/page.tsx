...
export default function SignupPage() {
  ...
  useEffect(() => {
    if (status === 'authenticated') router.replace('/setup')
  }, [status, router])
      ...
      <div className="relative w-full max-w-sm">
        ...
        <div className="bg-slate/40 border border-white/10 rounded-2xl p-8 backdrop-blur-sm shadow-2xl">
          <div className="text-center mb-6">
            ...
            <p className="text-slate-400 text-sm">Start practising interviews for free</p>
          </div>

          {error && (
            <div className="mb-5 px-4 py-3 rounded-lg bg-rose/10 border border-rose/30 text-rose text-sm text-center">
              {error}
            </div>
          )}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-3 px-6 py-3 rounded-xl border border-white/15 bg-white/5 hover:bg-white/10 text-white font-semibold transition-all disabled:opacity-50 disabled:cursor-not-allowed mb-5"
          >
            {googleLoading ? (
          ...
          <form onSubmit={handleSignup} className="space-y-4">
            <div>
              ...
              <input
                ...
                className="w-full px-4 py-3 rounded-xl bg-white/5 border border-white/10 text-white placeholder-slate-600 focus:outline-none focus:border-blue/50 transition-colors text-sm"
              />
            </div>
            <div>
            ...
            <button
              ...
              className="w-full py-3.5 rounded-xl bg-blue hover:bg-blue/90 text-white font-semibold transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {isLoading ? (
          ...
          <p className="mt-4 text-center text-xs text-slate-600 leading-relaxed">
            ...
            <Link href="/terms" className="text-slate-500 hover:text-white underline underline-offset-2">Terms</Link>
            {' '}and{' '}
            <Link href="/privacy" className="text-slate-500 hover:text-white underline underline-offset-2">Privacy Policy</Link>.
          </p>
        </div>
